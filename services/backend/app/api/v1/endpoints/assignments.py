...
from app.models_extended import Assignment, Grade, Notification, Submission
...
def _enrolled_in_course(db: Session, user: User, course_id: int) -> bool:
    return (
        db.query(Enrollment).filter(Enrollment.course_id == course_id, Enrollment.student_id == user.id).first()
        is not None
    )
...
def list_assignments(
    course_id: int | None = None, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def submit_assignment(
    assignment_id: int, body: SubmissionCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    ...
    existing = (
        db.query(Submission).filter(Submission.assignment_id == assignment_id, Submission.student_id == user.id).first()
    )
    if existing:
...
def grade_submission(
    ...
    s.status = "graded"
    s.graded_at = datetime.now(UTC)
    # Create notification
    db.add(
        Notification(
            user_id=s.student_id,
            title="Assignment Graded",
            message=f"Your submission scored {body.score}. Feedback: {body.feedback[:200]}",
