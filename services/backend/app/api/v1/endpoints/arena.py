...
from app.core.arena_engine import (
    ARENA_MODES,
    query_multiple_sources,
    score_answer,
    select_best_answer,
)
...
async def arena_query(
    ...
    try:
        ...
        if not answer or len(answer) < 20:
            ...
            try:
                from app.api.v1.endpoints.kudos import generate_answer, search_chunks
                ...
                answer = f"I couldn't find an answer to '{body.question}' right now. Try teaching me about this topic by uploading a document or web page."  # noqa: E501
        ...
        return {
            "answer": answer,
            "mode": mode,
            "mode_name": ARENA_MODES[mode]["name"],
            "mode_icon": ARENA_MODES[mode]["icon"],
            "best_source": result["source"],
            "confidence": result["score"],
            "alternatives": result.get("alternatives", []),
            "sources_queried": len(answers),
            "conversation_id": conv.id,
        }
...
async def arena_compare(query: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
        ...
        return {
            ...
            "best_source": comparisons[0]["source"] if comparisons else "none",
...
async def arena_quick_learn(topic: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
            ...
            if a["content"] and len(a["content"]) > 100:
                db.add(
                    KudosWebKnowledge(
                        ...
                        content=a["content"],
                        summary=simple_summarize(a["content"]),
        ...
        return {
            "topic": topic,
            "sources_found": len(answers),
            "items_stored": stored,
            "message": f"KUDOS learned about '{topic}' from {stored} sources via Arena",
        }
    except Exception as e:
        db.rollback()
